...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const modeToggleButton = document.querySelector('.js-color-mode-toggle');
  const modeToggleText = document.querySelector('.js-color-mode-toggle-text');
  const modeStatusElement = document.querySelector('.js-color-mode-status');
  /**
  ...
  const getCSSCustomProp = propKey => {
    let response = getComputedStyle(document.documentElement).getPropertyValue(propKey); // Tidy up the string if there’s something to work with
  ...
  const setButtonLabelAndStatus = currentSetting => {
    modeToggleText.innerText = `Enable ${currentSetting === 'light' ? 'dark' : 'light'} mode`;
